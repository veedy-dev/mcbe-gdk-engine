...
static HRESULT dialog_filters_create( struct file_picker_operation *operation, struct dialog_filters *out )
{
    ...
    out->count = operation->filter_count ? operation->filter_count + 1 : 1;
    ...
    if (!(out->patterns = calloc( operation->filter_count, sizeof(*out->patterns) )))
        goto oom;
    out->pattern_count = operation->filter_count;
    for (i = 0; i < operation->filter_count; ++i)
    {
        value = WindowsGetStringRawBuffer( operation->filters[i], &length );
    ...
    if (operation->filter_count == 1 && !wcscmp( out->patterns[0], L"*" ))
    {
        out->count = 1;
        out->specs[0].pszName = all_files;
        return S_OK;
    ...
    *cursor = 0;
    out->specs[operation->filter_count].pszName = all_files;
    out->specs[operation->filter_count].pszSpec = out->combined;
    return S_OK;
...
static HRESULT show_file_dialog( struct file_picker_operation *operation, IPickFileResult **result )
{
    ...
    *result = NULL;
    if (FAILED(hr = dialog_filters_create( operation, &filters ))) return hr;
